...
function Application(domain, path) {
  ...
  this.config = utils.parseConfig.call(this);
  ...
  framework.util.requireAllTo(this.path + "/lib", this.lib, requireCb = function(Func, prototype) {
    ...
    Func = utils.addClassInheritance.call(self, Func); // Enable this.extends within function
    var instance = new Func(self); // Instantiate
    instance.constructor = Func;
    if (typeof instance.__construct == 'function') instance.__construct.call(instance, self);
    instance.className = Func.name;
    return instance;
  });
  
  // Get helpers/
  this.helperProto = this.lib.helper || new framework.lib.helper(this);
  framework.util.requireAllTo(this.path + "/helpers", this.helpers, function(func) {
    return requireCb.call(self, func, self.helperProto);
  });
  
  // 1. Create storages (used by drivers & app)
  utils.createStorages.call(this);
  
  // 2. Create drivers (used by models)
  utils.createDrivers.call(this);
  
  // 3. Get models/
  this.modelProto = this.lib.model || new framework.lib.model(this);
  var model,name;
  framework.util.requireAllTo(this.path + "/models", this.models, function(func) {
    model = requireCb.call(self, func, self.modelProto);
    // Attach model to app. E.g: app.usersModel, app.dataModel, etc.
  ...
  // Get app:engines/
  this.engineProto = this.lib.engine || new framework.lib.engine(this);
  framework.util.requireAllTo(this.path + "/engines", this.engines, function(func) {
    return requireCb.call(self, func, self.engineProto);
  });
  ...
  for (i=0; i < files.length; i++) {
    ...
    ControllerClass = utils.monkeyPatchController.call(this, ControllerClass);
  ...
  files = utils.getStaticDirs.call(this);
...
Application.prototype = {
   ...
   */
   ...
   enable: function(feature, config) {
  ...
  registerEnable: function(feature, callback) {
  ...
  use: function(component, options, skipCheck) {
  ...
  routeRequest: function(req, res) {
    ...
    var method, queryData,
      urlData = utils.utils.parseUrl(req.url),
  ...
  serveStatic: function(path, req, res) {
  ...
  validate: function(req, fields, onlyCheck) {
  ...
  badRequest: function(res, logData) {
    this.loadCookies(res.__request);
    var cb = function() {
  ...
  serverError: function(res, logData) {
    var cb, self = this;
  ...
  rawServerError: function(res, message, logData) {
    if (logData == null) {
      logData = message;
      message = null;
    }
  ...
  loadCookies: function(req) {
    if (req.__cookies != null) return;
    return req.__cookies = utils.utils.getRequestCookies(req);
  },
   ...
   */
   ...
   getClassInstance: function(cname) {
  ...
  buildPartialView: function(path) {
  ...
  storage: function(storage, config) {
  ...
  createMulti: function(object, options) {
