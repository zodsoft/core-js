...
framework.extend(Application.prototype, new function() {
  ...
  // Method to run on initialization (override if necessary)
  ...
  this.__construct = function(domain, path) {
    ...
    // Get helpers/
    var helperProto = self.lib.helper || new framework.lib.helper(self);
    framework.util.requireAllTo(this.path + "/helpers", this.helpers, function(func) {
      return requireCb.call(self, func, helperProto);
    });
    ...
    // Get app:engines/
    var engineProto = self.lib.engine || new framework.lib.engine(self);
    framework.util.requireAllTo(this.path + "/engines", this.engines, function(func) {
      return requireCb.call(self, func, engineProto);
    });
    ...
    // Post Initialization
    this.once('post_init', function() {
      // Get models/
      var modelProto = self.lib.model || new framework.lib.model(self);
      framework.util.requireAllTo(this.path + "/models", this.models, function(func) {
        return requireCb.call(self, func, modelProto);
      });
    });
  ...
  this.log = function(context, msg) {
    ...
    var logger = (context.indexOf('[SERVER ERROR]' == 0)) ? console.trace : console.log;
    if (msg == null) {
      logger("[" + this.domain + "] - " + (this.date()) + " - " + context);
    } else {
      logger("[" + this.domain + "] - " + (this.date()) + " - " + context + " - " + msg);
    }
  }
