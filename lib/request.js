...
_.extend(IncomingMessage.prototype, new function() {
  
  this.__saveSession = null;
  this.__sessionQueue = {};
  ...
  this.saveSessionState = function(callback) {
    ...
    if (session.user != null) {
      expires = (session.pers ? app.config.session.permanentExpires : app.config.session.temporaryExpires);
    } else {
      expires = app.config.session.guestExpires;
    }
  ...
  this.sessionChanged = function() {
    var app = this.__app;
    
    if (typeof app.session == 'undefined') return false;
  ...
  this.getPostData = function(callback) {
    var req = this,
      res = this.__response,
      app = this.__app,
      form;
  ...
  this.exceededUploadLimit = function() {
