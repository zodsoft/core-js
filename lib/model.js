...
framework.extend(Model.prototype, new function() {
  ...
  this.__createModel = function(ob) {
    ...
    for (key in ob) {
      descriptor[key] = {value: ob[key], enumerable: true, configurable: true};
    }
    
    // Create __currentState object
    descriptor['__currentState'] = {value: ob, enumerable: false, configurable: false}
    
    // Create ModelObject
    ob = Object.create(proto, descriptor);
    
    // Freeze object current state & Prototype
  ...
  this.__validateProperties = function(o) {
    var key, val, regex, prop, validates, required, len,
        badProperties = [],
        app = this.app,
        properties = this.properties,
        unableToValidate = "%s: Unable to validate '%s': %s";
    
    // Check required fields by default
    checkRequired = (typeof checkRequired == 'undefined') ? true : checkRequired;
    ...
    for (key in properties) {
      ...
      if (checkRequired && required && !o.hasOwnProperty(key)) {
      ...
      if (key in o && typeof validates != 'undefined') {
        ...
        if (validates instanceof RegExp) {
          ...
          if (! validates.test(val)) {
            throw new Error(util.format(unableToValidate, this.className, key, val));
          }
          ...
          throw new Error(util.format("%s: Wrong validation data for '%s': %s", this.className, key, validates));
        }
      }
    }
  }
