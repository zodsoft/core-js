...
_.extend(OutgoingMessage.prototype, new function() {
  ...
  this.render = function(view, data, raw) {
    var self = this,
        app = this.__app,
        cacheStore = app.storage.response || app.storage.cache;
  ...
  this.useCache = function(cacheID) {
    var app = this.__app, cacheStore = app.storage.response || app.storage.cache;
    if (app.responseCaching === true) {
      if (cacheStore === null) app.log('Response Cache: `app.storage.(response|cache)` is not set');
  ...
  function asyncRender(view, data, raw) {
    ...
    if (app.session) data.session = this.__request.__session;
  ...
  function renderViewBuffer() {
    var buffer, codeBlock, controller, data, logData, relPath, 
